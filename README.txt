...
2. triv.c
	...
	output : str
        ...
        함수 설명 : if, else문과 for문을 이용해 함수를 구현했다.
                    d가 음수일 때와, d가 음수가 아닐 때(양수이거나 0일때)로 나눠서 d가 음수일때는 d=0-d를 이용하여 -를 뗸 형태로 바꾸고 반복문(for문)을 시작했다.
                    음수일 때, 문자열은 반시계방향으로 회전하도록 설정했다. 문자열을 str이라고 했을 때, str[1]부터 str[n-1]까지는 앞으로 한 칸씩 이동하면 되지만,
                   str[0]이 str[n-1]으로 이동하는 것은 같은 반복문으로 실행이 되지 않으므로 먼저 str[n-1]에 str[0]을 대입한 후, str[0]을 제외한 다른 문자들을 앞으로 한 칸 씩 이동시키는 for문을 실행한다. 
                   이 모든 과정을 d번 반복하는 for문을 실행한다. 
                    음수가 아닐 때에는, 음수일 때와 반대로 문자열은 시계방향으로 회전하도록 설정했다. 문자열을 str이라고 했을 때, 
                   str[0]부터 str[n-2]까지는 뒤로 한 칸씩 이동하면 되지만, str[n-1]이 str[0]으로 가는 것은 같은 반복문으로 실행이 되지 않으므로 미리 str[0]에 str[n-1]을 저장하고, str[n-1]을 제외한 모든 다른 문자들을 뒤로 한 칸씩 이동시키는 for문을 실행한다. 
                   음수일 때와 마찬가지로 이 모든 과정을 d번 반복하는 for문을 실행한다.
